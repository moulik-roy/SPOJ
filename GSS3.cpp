...
void update(node seg_tree[], long int index, long int val, long int i, long int s, long int e){
	if(s==index && e==index){
		seg_tree[i].prefix_sum=val;
		seg_tree[i].suffix_sum=val;
		seg_tree[i].sum=val;
		seg_tree[i].max_subarray_sum=val;
		return;
	}
	long int mid=s+(e-s)/2;
	if(index<=mid)
		update(seg_tree, index, val, 2*i+1, s, mid);
	else
		update(seg_tree, index, val, 2*i+2, mid+1, e);
	seg_tree[i]=merge(seg_tree[2*i+1], seg_tree[2*i+2]);
}
